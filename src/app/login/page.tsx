  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        successMessage.className = "bg-green-50 text-green-600 p-3 rounded-md text-sm"
    ...
    <div className="min-h-screen bg-[#F5F5F5] flex items-center justify-center p-4">
      <Card className="w-full max-w-md border-0 bg-white shadow-sm">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl font-bold text-[#5D4037] mt-4">로그인</CardTitle>
          <CardDescription className="text-[#33691E]">계정에 접속하려면 아이디와 비밀번호를 입력하세요</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <Button type="submit" className="w-full bg-[#795548] hover:bg-[#5D4037] mt-2" disabled={isLoading}>
              {isLoading ? (
                <>
        ...
        <CardFooter className="flex flex-col space-y-2">
          <div className="text-center text-sm text-[#33691E]">
            계정이 없으신가요?{" "}
            <Link href="/register" className="text-[#5D4037] font-medium hover:underline">
              회원가입
            </Link>
          </div>
        </CardFooter>
