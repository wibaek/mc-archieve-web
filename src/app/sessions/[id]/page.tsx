...
import type { Session, SessionMember, SessionJoinRequest, Story } from "@/types/session"
...
export default function SessionDetailPage() {
  ...
  const fetchMembers = async () => {
    try {
      ...
      const response = await apiClient.sessions.getSessionMembers(id)
      ...
      if (response.success) {
        ...
        // 내 역할 확인 (로그인한 경우에만)
        if (user) {
          const myMembership = response.data.items.find((member) => member.userId === user.id)
          if (myMembership) {
  ...
  const fetchJoinRequests = async () => {
    ...
    try {
      ...
      const response = await apiClient.sessions.getJoinApplications(session.id)
      ...
      if (response.success) {
        ...
        // 내 참가 요청 확인
        if (user) {
          const myRequest = response.data.find((request) => request.userId === user.id)
  ...
  const fetchStories = async () => {
    ...
    try {
      ...
      const response = await apiClient.sessions.getSessionStories(session.id)
  ...
  const handleJoinRequest = async () => {
    ...
    try {
      ...
      // 새로운 API 엔드포인트 사용
      const response = await apiClient.sessions.joinSession(session.id)

      if (response.success) {
        setHasJoinRequest(true)
        alert("참가 요청이 성공적으로 전송되었습니다.")
      } else {
        alert(response.message || "참가 요청 전송 중 오류가 발생했습니다.")
      }
    } catch (error) {
  ...
  const handleRespondToRequest = async (requestId: string, accept: boolean) => {
    ...
    try {
      let response
      if (accept) {
        response = await apiClient.sessions.approveJoinRequest(session.id, requestId)
      } else {
        response = await apiClient.sessions.rejectJoinRequest(session.id, requestId)
      }
  ...
  const handleRemoveMember = async (userId: string) => {
    ...
    try {
      const response = await apiClient.sessions.removeMember(session.id, userId)

      if (response.success) {
        fetchMembers()
      } else {
        alert(response.message || "멤버 제거 중 오류가 발생했습니다.")
      }
    } catch (error) {
  ...
  const handleLeaveSession = async () => {
    ...
    try {
      const response = await apiClient.sessions.leaveSession(session.id)

      if (response.success) {
        router.push("/sessions")
      } else {
        alert(response.message || "세션 나가기 중 오류가 발생했습니다.")
      }
    } catch (error) {
  ...
  const handleDeleteSession = async () => {
    ...
    try {
      const response = await apiClient.sessions.deleteSession(session.id)

      if (response.success) {
        router.push("/sessions")
      } else {
        alert(response.message || "세션 삭제 중 오류가 발생했습니다.")
      }
    } catch (error) {
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <Card className="border-0 shadow-sm overflow-hidden mb-8">
          <CardHeader className="flex flex-row items-start justify-between">
            <div>
              <CardTitle className="text-2xl font-bold text-[#5D4037]">{session.name}</CardTitle>
          ...
          <CardContent>
            {session.description && <p className="text-[#33691E] mb-6">{session.description}</p>}
        ...
        {!isAuthenticated && (
          <Card className="border-0 shadow-sm mb-8 bg-blue-50">
            <CardContent className="p-4">
              <p className="text-blue-700 mb-2">세션에 참가하거나 더 많은 기능을 사용하려면 로그인이 필요합니다.</p>
        ...
        {isAuthenticated && !isMember && !hasJoinRequest && (
          <Card className="border-0 shadow-sm mb-8">
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-[#5D4037]">세션 참가 요청</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        {hasJoinRequest && (
          <Card className="border-0 shadow-sm mb-8 bg-yellow-50">
            <CardContent className="p-4">
              <p className="text-yellow-700">이 세션에 참가 요청을 보냈습니다. 소유자의 승인을 기다리고 있습니다.</p>
            </CardContent>
          </Card>
        )}
        ...
        <Tabs defaultValue="members">
          ...
          <TabsContent value="members">
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-[#5D4037]">멤버 목록</CardTitle>
              </CardHeader>
              <CardContent>
                {membersLoading ? (
                  ...
                  <p className="text-center py-8 text-gray-500">아직 멤버가 없습니다.</p>
                ) : (
                  <div className="space-y-4">
                    {members.map((member) => (
                      <div key={member.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-md">
                        <div className="flex items-center">
                          <div className="ml-3">
                            <p className="font-medium text-[#5D4037]">{member.user.username}</p>
                            <p className="text-xs text-gray-500">
                              {member.role === SessionMemberRole.OWNER ? "소유자" : "멤버"} • 가입일:{" "}
          ...
          {isOwner && (
            <TabsContent value="requests">
              <Card className="border-0 shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg font-semibold text-[#5D4037]">참가 요청</CardTitle>
                </CardHeader>
                <CardContent>
                  {requestsLoading ? (
                    <div className="flex justify-center py-8">
                    ...
                    <p className="text-center py-8 text-gray-500">아직 참가 요청이 없습니다.</p>
                  ) : (
                    <div className="space-y-4">
                      {joinRequests.map((request) => (
                        <div key={request.id} className="p-4 bg-gray-50 rounded-md">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              <p className="font-medium text-[#5D4037]">{request.user.username}</p>
                              <p className="text-xs text-gray-500">요청일: {formatDate(request.createdAt)}</p>
                            </div>
                            <div className="flex space-x-2">
                              <Button
                                variant="outline"
                          ...
                          {request.message && (
                            <div className="mt-2 p-3 bg-white rounded border border-gray-100">
                              <p className="text-sm text-[#33691E]">{request.message}</p>
                            </div>
                          )}
                        </div>
                      ))}
          ...
          <TabsContent value="stories">
            <Card className="border-0 shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-lg font-semibold text-[#5D4037]">스토리</CardTitle>
                {isMember && (
              ...
              <CardContent>
                {storiesLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    {isMember && (
                      <Button asChild className="mt-4 bg-[#33691E] hover:bg-[#1B5E20]">
                        <Link href={`/sessions/${session.id}/stories/create`}>
                  ...
                  <div className="space-y-4">
                    {stories.map((story) => (
                      <Link href={`/sessions/${session.id}/stories/${story.id}`} key={story.id}>
                        <div className="p-4 bg-gray-50 rounded-md hover:bg-gray-100 transition-colors">
                          <div className="flex justify-between items-start mb-2">
                            <h3 className="font-medium text-[#5D4037]">{story.title}</h3>
                            <span className="text-xs text-gray-500">{formatDate(story.createdAt)}</span>
                          </div>
                          <p className="text-sm text-[#33691E] line-clamp-2">{story.content}</p>
                          <div className="mt-2 text-xs text-gray-500">작성자: {story.user.username}</div>
                        </div>
                      </Link>
                    ))}
                  </div>
