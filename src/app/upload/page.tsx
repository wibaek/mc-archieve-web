...
export default function UploadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 업로드 성공 후 리다이렉트 경로 수정
      // 갤러리 페이지로 리다이렉트하는 부분을 홈페이지로 변경
      if (response.success) {
        ...
        setErrorMessage(response.message || "업로드 중 오류가 발생했습니다.")
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#F5F5F5] py-12 px-4">
      <div className="container mx-auto max-w-3xl">
        <Card className="border-0 bg-white shadow-sm">
          <CardHeader>
            <CardTitle className="text-2xl font-bold text-[#5D4037]">사진 업로드</CardTitle>
          ...
          <CardContent>
            ...
            {successMessage && (
              <div className="bg-green-50 text-green-600 p-3 rounded-md text-sm mb-4">{successMessage}</div>
            )}
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {!imagePreview ? (
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm text-gray-500">이미지를 선택하거나 여기에 드래그하세요</p>
                    <p className="text-xs text-gray-400 mt-1">PNG, JPG, GIF 파일만 지원됩니다 (최대 10MB)</p>
                    <Input
                      id="image"
                      type="file"
                      ref={fileInputRef}
              ...
              <Button type="submit" className="w-full bg-[#33691E] hover:bg-[#1B5E20]" disabled={isLoading}>
                {isLoading ? (
                  <>
          ...
          <CardFooter className="flex justify-center border-t border-gray-100 pt-4">
            <p className="text-xs text-gray-500">
              업로드한 콘텐츠는 MC Archive의 커뮤니티 가이드라인을 준수해야 합니다.
            </p>
          </CardFooter>
        </Card>
      </div>
